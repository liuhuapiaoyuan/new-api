...
// buildAdminLogFilterTx returns a DB handle with the same filters as the admin usage log list.
func buildAdminLogFilterTx(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, channel int, group string, requestId string, upstreamRequestId string) (*gorm.DB, error) {
	var tx *gorm.DB
	var err error
...
// fillLogChannelNames resolves channel display names for the given logs (admin / export).
...
func GetAllLogs(logType int, startTimestamp int64, endTimestamp int64, modelName string, username string, tokenName string, startIdx int, num int, channel int, group string, requestId string, upstreamRequestId string) (logs []*Log, total int64, err error) {
	tx, err := buildAdminLogFilterTx(logType, startTimestamp, endTimestamp, modelName, username, tokenName, channel, group, requestId, upstreamRequestId)
	if err != nil {
		return nil, 0, err
	}
...
// buildUserLogFilterTx returns a DB handle with the same filters as the self usage log list.
func buildUserLogFilterTx(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, group string, requestId string, upstreamRequestId string) (*gorm.DB, error) {
	var tx *gorm.DB
	var err error
	if logType == LogTypeUnknown {
